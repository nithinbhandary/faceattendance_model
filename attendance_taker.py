...
#create_table_sql = f"CREATE TABLE IF NOT EXISTS attendance (name VARCHAR(255),time TIME,date DATE,CONSTRAINT unique_name_date UNIQUE (name, date));"
...
class Face_Recognizer:
    ...
    def attendance(self, name):
        ...
        if existing_entry:
            ...
            cursor.execute("SELECT roll_no FROM studentdetails WHERE name=%s",(name,))
            rollno=cursor.fetchone()[0]
            cursor.execute("INSERT INTO attendance (name, time, date, roll_no) VALUES (%s, %s, %s,%s)", (name, current_time, current_date, rollno))
            conn.commit()
